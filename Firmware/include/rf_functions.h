        ...
        if (driver.recv(buf, &buflen))          //If got anything from RF receiver
        {
            ...
            if(message.indexOf(RF_password) > -1){
                ...
                if(RF_enable){
                    ...
                    mqttClient.publish(logPath, 0, true, "Got right RF password, opening the door");
                    openDoor(timeOpenDoor);
                }
                else{
                    SerialMon.println("RF is disabled tho");
                    mqttClient.publish(logPath, 0, true, "Got right RF password, but RF door opening is disabled");
                }
            }
            else{
                SerialMon.println("Passord NOT matching");
                mqttClient.publish(logPath, 0, true, "Got wrong RF password");
            }
        }
        vTaskDelay(1);
    }
